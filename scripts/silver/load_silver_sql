...
-- Loading silver.crm_cust_info
	...
	SET @start_time = GETDATE();
		...
		SELECT  
			...
			-- Ensure sales calculation consistency
			CASE 
	...
	SET @start_time = GETDATE();
		...
		SELECT
			CASE
				WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, DATALENGTH(cid) - 3) -- Remove 'NAS' prefix
				ELSE cid
			END AS cid, 
			CASE
				WHEN TRY_CONVERT(DATE, bdate) IS NULL THEN NULL -- Handle invalid dates
